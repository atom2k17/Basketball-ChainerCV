...
model = SSD300(n_fg_class=len(bball_labels))
...
def train(train_data, valid_dataset, label_names,
          iteration, lr, step_points,
          batchsize, gpu_id, out, val_iteration,
    ...
    if loaderjob <= 0:
        ...
        train_iter = chainer.iterators.MultiprocessIterator(
            train_dataset, batchsize, n_processes=min((loaderjob, batchsize)))

    val_iter = chainer.iterators.SerialIterator(
        valid_dataset, batchsize, repeat=False, shuffle=False)
...
train(train_data, valid_dataset, bball_labels,lr=0.001, batchsize=32, gpu_id=0, loaderjob=2)

train(train_data=train_iter, val_data=val_iter, label_names=bball_labels, iteration, lr=0.001, step_points, batchsize=32, gpu=0, out, val_iteration, log_iteration, loaderjob=2, resume)
...
trainer.extend(DetectionVOCEvaluator(valid_iter, model, use_07_metric=False,label_names=bball_labels),trigger=(1, 'epoch'))
trainer.extend(extensions.LogReport(trigger=log_interval))
trainer.extend(extensions.observe_lr(), trigger=log_interval)
...
if extensions.PlotReport.available():
    trainer.extend(extensions.PlotReport(['main/loss', 'main/loss/loc', 'main/loss/conf'],'epoch', file_name='loss.png'))
    trainer.extend(extensions.PlotReport(['validation/main/map'],'epoch', file_name='accuracy.png'))
